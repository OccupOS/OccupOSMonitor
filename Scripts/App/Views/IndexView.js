...
OccupOS.IndexView = Ember.ContainerView.extend({
        ...
        LinechartView: Ember.View.extend({
            ...
            didInsertElement: function didInsertElement() {
                ...
                if (this.get('parentView.parentView.sensors.isLoaded')) {
                    ...
                    //drawLineChart(this.get('parentView.parentView.sensors'), 1);
                    var sensors = this.get('parentView.parentView.sensors');
            //		var sensors = this.get('parentView.parentView.sensorsArray');
                    var id = 1;
    ...
    RowTwoView: Ember.ContainerView.extend({
        classNames: ['row'],
        childViews: ['LinecharttwoView'],
        LinecharttwoView: Ember.View.extend({
            classNames: ['span12'],
            templateName: 'linecharttwo',
            chart: {},
            line: {},
            graph: null,
            x: null,
            ...
            didInsertElement: function didInsertElement() {
                if (this.get('parentView.parentView.sensors.isLoaded')) {
                    ...
                    var svg = d3.select(".linecharttwo").append("svg")
                        .attr('class', 'graph')
                        .attr('height', height + margin.top + margin.bottom)
                      .append("svg")
                        //.attr("width", width + margin.left + margin.right)
